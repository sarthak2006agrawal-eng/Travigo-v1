...
const computeCostBreakdown = (daily_plan = [], existing = {}) => {
  let transport = 0, accommodation = 0, activities = 0;
...
export const createItinerary = asyncHandler(async (req, res) => {
  ...
  const { destination, start_date, end_date, budget, travel_style, trip_name } = req.body;

  if (!destination || !start_date || !end_date) {
    throw new ApiError("destination, start_date and end_date are required", 400);
  }
  ...
  const dest = await Destination.findById(destination).select("name country images");
  if (!dest) {
  ...
  const itinerary = await Itinerary.create({
    ...
    cost_breakdown: { transport: 0, accommodation: 0, activities: 0, total: 0, currency: "INR" },
    status: "draft"
  });
  ...
  return new ApiResponse(201, "Itinerary created successfully", payload).send(res);
});
...
export const getItineraryById = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) throw new ApiError("Invalid itinerary id", 400);
  ...
  if (itinerary.user.toString() !== userId.toString()) throw new ApiError("Not authorized", 403);

  return new ApiResponse(200, "Itinerary fetched", itinerary).send(res);
});
...
export const updateItinerary = asyncHandler(async (req, res) => {
  ...
  const allowed = ["trip_name", "start_date", "end_date", "budget", "travel_style", "daily_plan", "status"];
  const updates = {};
  for (const key of allowed) {
    if (req.body[key] !== undefined) updates[key] = req.body[key];
  }
  if (Object.keys(updates).length === 0) throw new ApiError("No updatable fields provided", 400);
  ...
  if (updates.start_date || updates.end_date) {
    const start = updates.start_date ? new Date(updates.start_date) : itinerary.start_date;
    const end = updates.end_date ? new Date(updates.end_date) : itinerary.end_date;
    if (isNaN(start) || isNaN(end) || start > end) throw new ApiError("Invalid dates", 400);
  ...
  if (updates.travel_style !== undefined) itinerary.travel_style = updates.travel_style;
  ...
  if (updates.daily_plan !== undefined) {
    if (!Array.isArray(updates.daily_plan)) throw new ApiError("daily_plan must be an array", 400);
    itinerary.daily_plan = updates.daily_plan;
    // recalc cost summary
    itinerary.cost_breakdown = computeCostBreakdown(itinerary.daily_plan, itinerary.cost_breakdown);
  }

  await itinerary.save();

  const result = await Itinerary.findById(itinerary._id).populate("destination", "name country images").lean();
